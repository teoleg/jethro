...
#    assumed. If the app did not come back on a process that started after the deploy began, fall back
#    to the repo's own restart, which is always correct on this box: it STOPS the app before rebuilding
#    (so the jar is never rewritten under a live JVM) and re-reads local.env, so provider/keys/profile
#    come back identical rather than reverting to run-local.sh's defaults — a silent feed switch is an
#    invariant-8 event, not a restart.
#
#    JETHRO_DEPLOY_CMD stays YOUR command; unset, the repo's own is the default rather than doing
#    nothing. Set JETHRO_DEPLOY_CMD=none for review-before-live (commit+push, never restart).
DEPLOY_FALLBACK_CMD="scripts/svc.sh restart app"
...
# it. Polls rather than sleeping blind: a cold start on this box takes a couple of minutes.
wait_for_restart() {
...
# 9>&- as below: the deploy starts the long-lived app (and may spawn Gradle); neither must inherit
...
if [ "$DEPLOY_CMD" = "none" ]; then
  ...
  if wait_for_restart "$DEPLOY_STARTED_AT"; then
    echo "deploy verified — the app is answering on a process started by this deploy" >> "$LOG"
  elif [ "$DEPLOY_CMD" != "$DEPLOY_FALLBACK_CMD" ]; then
    echo "DEPLOY NOT VERIFIED — '$DEPLOY_CMD' left the pre-change process running (or the app is down)." \
         "Falling back to the repo's own restart so the change actually reaches the JVM." >> "$LOG"
    run_deploy "$DEPLOY_FALLBACK_CMD"
    wait_for_restart "$DEPLOY_STARTED_AT" \
      && echo "fallback deploy verified — the app is answering on a freshly started process" >> "$LOG" \
      || echo "DEPLOY FAILED — the app is still on the pre-change binary. The next cycle's verdict will" \
              "describe code that never ran; fix the deploy before trusting it." >> "$LOG"
  else
    echo "DEPLOY FAILED — the app is still on the pre-change binary. The next cycle's verdict will" \
         "describe code that never ran; fix the deploy before trusting it." >> "$LOG"
  fi
