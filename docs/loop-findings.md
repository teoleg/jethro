...
### 2026-07-27T01:45Z — ADR-0088 (the conviction is averaged over the horizon it is graded on)
- Situation: PnL essentially flat (`-792.49 → -792.34`, `+33.72` over three runs, `on_track`) while gross
  exposure went `130.69 → 21,142.84` in ONE window. Not a breaker danger state (historical VaR95 `298.78`)
  but the objective — PnL per unit of exposure — moved hard the wrong way, and it was not finished: MSFT's
  planned target was `220.73` shares against `21.0` held, grinding up every cycle. Attribution is clean:
  the non-MSFT lines were flat or reduced, so ~100% of the move was the reverted change plus the ratchet.
- ADR-0087 (remove the source-weight floor) scored ❌ BAD and was auto-reverted — correctly, and for the
  exact reason I flagged when shipping it: uncancelling `reversion` against `trend` raises forecast
  magnitude and so the book. **Rule: when you predict in writing that a change will raise exposure, that
  is not a caveat, it is a reason not to ship it in a window where exposure is already the binding
  problem.** The prediction was right and I shipped it anyway.
- The window's tape was ONE name: **74 MSFT fills** for a net change of 21 shares — BUY 8 → BUY 4 → BUY 5 →
  BUY 6 → BUY 2 → BUY 3 → BUY 1 → SELL 16 → SELL 6 → SELL 9 → SELL 2 → SELL 1 → BUY 16 → SELL 4 — a sign
  flip every 3-5 minutes against a **900 s** selected rung, at ~1.0 bp of fee per leg, with the HEDGE book
  shorting ES against every swing (which is why GROSS doubled: MSFT `+9,001` / ES `-8,950`).
- Change: one EWMA per name on the COMBINED forecast, time constant = the ADR-0082 rung,
  `α = 1 - exp(-Δt/h)` — ADR-0080's own identity applied to the signal instead of to the trade. No dial,
  no money number. Constant forecast ⇒ unchanged; ±20 square wave at a 4-min half-period ⇒ `2.65`, under
  the never-triggered ADR-0059 floor of 5.0.
- Lesson / rule: **an identity applied to one side of a control loop is only half applied.** ADR-0080 fixed
  the POSITION's time constant and left the SIGNAL's free, and because its reducing leg is instantaneous by
  design, a fast-reversing signal still dragged the book at full speed. When you tie one quantity to a
  measured horizon, ask immediately what OTHER quantity in the same loop is still running at its own rate.
- Rule 2: **a no-trade band sized as a fraction of the TARGET is silent exactly when the target moves most.**
  `band = |target| × 0.5` is tested against a gap that, on a sign flip, equals `|target| + |current|`. It
  suppresses small corrections and does nothing about reversals — which are where the turnover actually is.
  Check what a guard does at its input's LARGEST excursion, not its typical one.
- Rule 3: **a forecast at its cap is a forecast with no size information.** Both sensors read exactly ±20 on
  the only tradable name. Whenever a signal is pinned, sizing has stopped being a function of evidence, and
  every downstream control that reads magnitude (the conviction floor, the vol budget, the diversification
  multipliers) is being fed a constant. Filed as a deferred row — measure cap frequency per sensor before
  touching any normalisation scalar.
- Honest limitation: the filter LAGS a genuine reversal, deliberately. If the ledger shows the desk
  consistently late on real turns, the fix is a shorter selected rung (the ladder will find it if the fast
  edge is real), NOT a hand-tuned filter. And the signal telemetry still grades the RAW sensor call, so the
  gate measures a series the desk no longer trades verbatim — deferred row filed; that is the first thing to
  look at if the verdict and the realised fusion PnL disagree.
