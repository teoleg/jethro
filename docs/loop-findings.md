...
### 2026-07-27T01:15Z — ADR-0087 (source trust has no floor)
- Situation: **the desk is healthy for the first time in the run.** PnL `-788.14 → -787.09` run-over-run
  and `+$38.97` over three runs (`pnl_growth_pct` 4.59% vs a 1% target, `on_track`); gross exposure
  `$1,603.06 → $130.67`, VaR95 `$1.99`, breaker far away. 55 fills today: **MACRO/ES `+$33.68` on `$2.36`
  of fees and HEDGE/ES `+$68.88` — both winners.** ALPHA's `-$889.65` is seven equity names all flat with
  zero unrealised: realised churn from the 7/26 session, sunk before this window and unmoved in it.
- Last cycle's ADR-0086 chandelier scored ⚠️ MIXED and **was right**: gross `$6,495.71 → $130.69` with PnL
  up. The MIXED is the risk-adjusted ratio flipping sign-wise when a *negative* PnL is divided by a much
  smaller gross. **Rule: PnL/$1 gross is meaningless while PnL is negative — de-risking always "worsens"
  it. Read the two legs of the vector separately when the book is underwater; the ratio lies.**
- Change: with the book trading, the binding question moved from *whether* to trade to *what view* the
  trades carry. `trend` is measured significantly NEGATIVE at every rung of the ladder (230–500 resolved
  calls, 10–22 cohorts, negative at 3600/900/225s) — evidence-implied weight **0.088** — and
  `weights.min=0.25` was lifting it to 0.25, **2.84× its own measurement**, while no other source came
  near the clamp. Removed the floor (ADR-0087); Bühlmann shrinkage is now the single continuous
  thin-sample defence, exactly as ADR-0074 argued when it removed this floor's twin.
- Lesson / rule: **a clamp that binds on exactly one input is not a safety rail, it is an override of the
  measurement — and it will always bind on the worst-measured one.** Before trusting a bound, check *which*
  inputs it actually touches on the live data. A bound that never binds costs nothing; one that binds on a
  single input is a hand-placed opinion wearing a parameter's clothes.
- Rule 2: **when a justification is retired, the thing it justified does not retire itself.** The floor's
  stated reason (preserve the source count for the diversification multiplier) died with ADR-0076 five
  cycles ago, and the floor kept running. When an ADR replaces a mechanism, grep for what was defended by
  the old one — the leftovers do not announce themselves.
- Rule 3: **`trend` and `reversion` read the same mark stream and are mirrors** (hit rate 0.290 vs 0.785 at
  900s; `trend` opposes `reversion` on all 12 of the largest planned names). Blending mirrored sources is
  netting, not diversification — the weights decide which one wins, so getting them wrong is not a haircut,
  it is a partial cancellation of the only edge the desk has.
- Honest limitation: conviction is what `TargetPlanner` sizes from, so **gross exposure will rise** on the
  two names that clear the ADR-0075 cost test (ES, MSFT). If the next verdict is ❌ BAD, what is refuted is
  `reversion`'s live edge, **not** the floor's redundancy — the next lever is then the COST side, not the
  weights, and do not re-attempt this one.
- Still open (fifth cycle running): the desk plans **~$550k of notional across 12+ names and executes
  `$130`** — every one of the 12 largest targets shows `deltaQty: 0`, clamped reduce-only by the ADR-0075
  per-name cost test. At `reversion`'s measured edge the largest survivable round trip is ~3.5 bps, and
  measured slippage says only ES (0.13 bps/fill) and MSFT (1.21) are under it; JNJ 1.94, JPM 2.09, AAPL
  2.10, GOOG 2.98, SAP 4.08, GOOGL 10.05 are all out. **That is the real cap on this desk's size**, and it
  is a COST problem, not an evidence problem. Do not attack it by loosening the hurdle.
