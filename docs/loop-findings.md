...
## 2026-07-27 — a weight must be proportional to the EDGE, not to the confidence in it (ADR-0093)

- **Situation.** Third consecutive up run: PnL `-$54.81 → $14.33 (+$69.14)` scored, `$326.15` over three
  runs, gross `$22,261` and falling (`-$7,107` on the window, `-$8,405` over three), VaR95 `$187.23`,
  breaker far. No danger state, so the cycle went at a mechanism rather than a de-risk.
- **Honest attribution on ADR-0092's ✅ GOOD.** The exposure half is the change's own doing — it rescaled
  over-large forecasts down, so the planned book shrank and the held book with it. The PnL half is `$69`
  against a `$50` deadband, barely outside noise; credited only weakly, not claimed as proof.
- **Closed the lever the last finding named.** The previous cycle flagged the ADR-0089 stream covariance
  as the next suspect because the boot log showed it cold. It is not: the live target book reports
  `covarianceBasis = mark-stream`, `covarianceCoveredNames = 23`, `streamVolMeasuredNames = 23`,
  `portfolioRiskMultiplier = 0.358`. **Rule: a seed-time WARN is a one-shot, not a state.** Before
  building on "X is cold", read the live disclosure field that reports whether X is cold NOW — the boot
  log only says what was true at boot, and both sensors warm within a cycle of it.
- **The finding.** Source weight was `Φ(t_s)` — the confidence a source's true expectancy is positive.
  Two defects that compound. `Φ` puts the null at exactly ½ and the normaliser turns ½ into a full unit
  of weight, so `mean-reversion` at `+0.28 bps` against a `±8.0 bps` standard error over six calls
  carried `1.055` — 49% of `reversion`, measured `t = +9.0` over 483 calls and 24 cohorts. And `Φ`
  saturates (`Φ(2.0) = 0.977` vs `Φ(9.4) = 1.000`), so an overwhelming edge bought 2.3% more weight than
  a marginal one. The four sources that FAIL the desk's own significance test held `3.08` of the `5.25`
  weight mass — 59% — and since the combiner normalises by `Σw`, the desk's only proven edge reached the
  book at 41% strength. Fixed by weighting on the shrunk measured edge, `max(0, c·µ̂ + (1−c)·pool₋ᵢ)`.
- **Rule: a statistic bounded on (0,1) cannot express a weight.** Anything that maps "no evidence" to a
  non-zero constant will pay a source for showing up, and anything that saturates will stop paying the
  best source for being better. **Check both ends of every link function: what does it return at the
  null, and where does it stop responding?** The tell sat in `fusion_targets` `weights` for ten cycles —
  a source with six observations sitting at half the weight of one with 483.
- **Rule 2: on an unbounded scale, shrink toward a LEAVE-ONE-OUT prior.** A pool containing the source
  itself lets a thin extreme reading drag the pool toward itself and then be shrunk toward its own luck.
  Under `Φ` this was invisible because everything saturated near 1; the moment the scale opened up it
  turned a `+80 bps`-on-three-calls source into an up-weight. Leaving it out flips that to `0.938`.
- **Did NOT re-attempt** ADR-0087 (remove the MIN floor — ❌ BAD, reverted) or ADR-0088 (EWMA the
  conviction — ❌ BAD, reverted). The floor is untouched by this change; only the link function moved.
- Expected next: `reversion` at `MAX` and everyone else at or near `MIN`, so the book becomes a bet on
  one measurement. If the ledger scores this ❌ or the book concentrates dangerously, the lever is the
  `MIN`/`MAX` band — tuned against a saturating statistic and probably wrong for a non-saturating one —
  **not** another link-function change.
- Still open, unchanged from last cycle: `MACRO` and `HEDGE` hold offsetting `ES` legs that cancel at the
  firm level for gross and ~0 net. Cheap, so logged rather than chased; the fix when it is worth doing is
  netting the hedge target against the proxy the firm holds ANYWHERE, not just in the hedge book.
