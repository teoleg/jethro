...
### 2026-07-27T00:20Z — ADR-0085 (the OOS gate measures each instrument in its own terms)
- Situation: fourteenth consecutive zero move — PnL frozen, gross and net exposure exactly zero, no
  orders in the window. Attribution is exact and empty: 0% market, 0% change. Not a danger state
  (nothing held to cut); a hard miss on the growth target and the sixth change left unmeasurable by a
  book that will not trade.
- **Two of the desk's own gates had opposite opinions about the same name, and one of them was arithmetic.**
  The ADR-0075 edge gate tests each name against its own measured round trip and clears exactly one —
  ES, the cheapest name in the book — and the planner produces a non-zero ES delta every cycle. The
  ADR-0049 backtest-support veto then discards it for having *no verdict*. ES has no verdict because
  `BacktestEngine` sized every name in whole units: ES's correct 0.091743 contracts rounded to 0, the
  code read "one contract exceeds the order cap" and declared it unsizeable. NO future had ever been
  measured — 17 of 19 price-quoted names, all equity/FX. The harness also charged every name the
  EQUITY per-fill cost, 8.1× what the live executor charges ES.
- Change: the backtest sizes in the instrument's own contract terms (ADR-0078's rule, applied to the
  measurement path) and charges each name its own cost (half its refdata spread + class fee — the
  derivation `OrderConfig` already hands the live `SimulatedExecutor`). Replaying the selector's
  harness: ES 0 trades/no verdict → 8 trades, positive momentum median; NQ trades but medians to zero
  and stays vetoed. No gate loosened, no hurdle moved, no number invented.
- Lesson / rule: **when a fail-closed gate is silent about a name, ask whether it CAN speak before
  believing it has nothing to say.** "No verdict" read as a considered refusal for fourteen cycles;
  it was an arithmetic overflow of a rounding rule. A fail-closed veto is only as good as the
  measurement behind it, and a measurement that is *structurally impossible* is invisible — it
  produces no error, no warning, no row: just an absence that looks like a decision.
- Rule 2: **when two controls disagree about the same name, that disagreement is the highest-value
  signal on the board.** One gate said "ES is the only name worth paying to trade", the other said
  "not ES". Neither is wrong on its own terms; the contradiction localised the bug in one step, after
  five cycles of tuning statistics that were never binding.
- Rule 3: **a correction applied on the live path must be applied to the path that MEASURES it.**
  ADR-0078 fixed contract-terms sizing on the order path and left the backtest rounding to whole
  units; from then on the harness was measuring a strategy the desk does not run. Same for cost: the
  javadoc claimed backtest and live "agree by construction" while the wiring passed `"EQUITY"` for
  every asset class. Grep for the live rule's twin whenever you fix a sizing or costing rule.
- Next lever if this is scored ❌ BAD *with fills happening*: the ADR-0075 cost term still charges
  `2 ×` one-way slippage measured under a policy where both legs crossed, though ADR-0084 made
  entries rest. That over-charges by exactly the leg the desk no longer pays, and it cannot
  self-correct (no fill ⇒ no new measurement). Real, but on its own it opens nothing: halving every
  measured cost still leaves the cheapest equity at roughly twice what the edge can carry.
- Still observed, still unmeasurable, fourth cycle running: **forecast saturation** — `trend` and
  `reversion` sit at the ±20 cap on ~a quarter of names each with opposite signs, so the cross-section
  carries little selection information.
