...
## 2026-07-27 — the desk was sizing a book it could never hold (ADR-0096)

- Situation: PnL up for the fifth run running — `216.91` total, `+32.13` on the window, `+100.81` over
  three, `on_track` true and the breaker far. But gross read `29,590.68` and then `40,891.67` eight
  minutes later against `19,038.18` at 05:50. **The numerator is fine and the denominator is on a ramp.**
  Attribution: ADR-0095 (❌ BAD, auto-reverted before I started) touched only the hedge, yet the exposure
  rise is in ALPHA too, predates it and continued after the revert — so I split nothing and claimed
  nothing for the PnL move. The ramp I acted on is visible independently, in the target book itself.
- **The finding.** `/api/ops/jvm` reports `selector: measured 17, tradable 7`; `/api/fusion/targets`
  reports `instruments: 23`. The ADR-0049 veto is asked **per order, at the last step, inside
  `FusionExecutor.route`** — so sixteen names the desk can never enter were planned, sized, and
  **aggregated over** every cycle. Free in fees (a vetoed order is never sent), which is why it survived
  nine cycles. Not free in sizing: ADR-0079's `PDM = min(1, σ_indep/σ_actual)` is computed over the
  planned book and applied UNIFORMLY, and a phantom name in a weakly-correlated asset class contributes
  `eᵢ²Σᵢᵢ` to `σ_indep` without contributing much to `σ_actual` — so it RAISES the measured
  diversification and the names that do trade are sized up on risk the desk was never going to take.
  Live `portfolioRiskMultiplier: 0.487` over `covarianceCoveredNames: 23`, with FX and futures carrying
  about half the planned notional. ADR-0083's harmonic-mean reference σ has the identical scope error.
- **Rule: a gate asked at the last step is invisible to every control above it.** The bind rate was not
  the problem here — this veto fires constantly — the *position* in the pipeline was. When a hard gate
  decides WHICH NAMES may carry risk, everything that aggregates over the book must be asked after it,
  or it is aggregating over names that will never exist. Check where each gate sits, not just that it
  fires. (Sibling of ADR-0092's "check the clip rate" and ADR-0094's "a control is dead until you have
  seen it fire": this one fired every cycle and was still in the wrong place.)
- **Rule 2: the order flow names the real book in one query.** `/api/orders/day` over 24 minutes: 100
  orders, all ALPHA activity in **five** names (AAPL, GOOG, JNJ, JPM, MSFT) against 23 planned. Comparing
  the traded name set against the planned name set is one line and would have caught this on day one.
  Do it every cycle before believing any book-level statistic.
- **Rule 3: an aim on the wrong side of zero is windup, and windup names the dead code.** `aims` showed
  `GS +103.02` against a target of `−87.51` and `BRK.B +49.57` against `−150.05`. The ADR-0094 buffer
  re-seeds its aim when the *edge gate* refuses but not when the *executor* does, so intent integrates
  forever on a name that can never fill. Costless here, but it is a free pointer at unreachable names —
  and it misled one earlier cycle's read of the target book.
- Expected next: gross stops ramping and falls with PnL on its trend. **The two effects do not point the
  same way** — removing the low-σ futures RAISES ADR-0083's harmonic reference and nudges the equity
  budgets up, partly offsetting the PDM cut — so if gross rises anyway this is the wrong lever, revert it
  rather than tune it, and go at the per-name budget itself rather than the set it is measured over.
- Do NOT re-attempt: the hedge's own sizing (ADR-0095, ❌ BAD, reverted) and the reversion sensor's
  Donchian/normalisation span (`8e80ceabc`, ❌ BAD, reverted) — both burned in the last two cycles.
  Source weighting stays burned from ADR-0087/0093. Still open, unclaimed: the executor-veto arm of the
  aim re-seed, and the fact that the loop's auto-revert also reverts `docs/loop-findings.md`, discarding
  the lessons of exactly the cycles worth remembering.
