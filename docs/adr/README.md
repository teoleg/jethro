...
| [0064](0064-cost-aware-edge-gate.md) | Gate risk-increasing fusion trades on measured edge net of measured cost | Superseded by [0067](0067-evidence-scaled-risk-appetite.md) | ✔ Was built, now replaced. Its binary `t ≥ hurdle` switch proved unsatisfiable by construction (`t = ((mean−cost)/σ)·√n` demands a Sharpe no real signal realises at this sample size), so "self-healing" never fired and the desk sat flat at zero. Its reduce-only projection survives in ADR-0067 for the case it was actually built for: *measured* harm |
...
| [0067](0067-evidence-scaled-risk-appetite.md) | Size the fusion book to the measured evidence (Φ of the t-stat), and stop only on measured harm — supersedes [0064](0064-cost-aware-edge-gate.md) | Proposed | ✔ Built (the edge gate returns a continuous risk appetite in [0,1] scaling `unit-notional-usd` before planning; Φ(0)=½ with no evidence, so it adds no dial and can only shrink the configured size; fully reduce-only retained for `t ≤ −t-hurdle`; conviction floor, backtest veto, guardrail and breaker all untouched) |
