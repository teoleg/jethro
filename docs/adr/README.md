...
| [0070](0070-factor-level-vol-gated-trend-strategy.md) | Factor-level vol-gated trend strategy — the edge is at the equity-FACTOR level (regime persistence + vol clustering), not per-name where idio noise dominates; go LONG the calm factor up-trend, CUT to flat when factor vol spikes or the trend turns down, HOLD otherwise. The owner's "risk sensor IS the trend sensor" thesis in code | Proposed | ◐ `FactorTrendStrategy` implemented + tested (long/cut/hold stances, basket=refdata EQUITY, statistical z/vol only); wired opt-in `jethro.strategy.factor-trend.enabled` DEFAULT OFF — awaits OOS validation before enable; live-tuning + short leg + non-equity factors deferred |
