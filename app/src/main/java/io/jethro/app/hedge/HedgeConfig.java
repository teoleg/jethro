...
public class HedgeConfig {
    ...
    HedgeAdvisor hedgeAdvisor(
            ...
            @Value("${jethro.hedge.equity-cap-usd:100000}") BigDecimal equityCapUsd,
            ...
            @Value("${jethro.hedge.fallback-beta:1.0}") BigDecimal fallbackBeta) {
        return new HedgeAdvisor(HedgeAdvisor.Mode.valueOf(mode.trim().toUpperCase(java.util.Locale.ROOT)),
                equityCapUsd, effectivenessFloor, equityProxy, equityProxyMultiplier, fallbackBeta);
    }
